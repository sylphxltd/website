...
try {
	...
	console.warn(
		`Parsing JSON content failed: ${parseError instanceof Error ? parseError.message : parseError}. Falling back to ADC...`,
	);
...
if (!initialized) {
	console.error(
		`Could not initialize Firebase Admin SDK. Last error: ${initError instanceof Error ? initError.message : initError}`,
	);
	process.exit(1);
}

<template>
  <div class="flex h-screen bg-gray-50 dark:bg-gray-900">
    ...
    >
      ...
      <!-- Navigation -->
      <nav class="flex-1 px-2 py-4 space-y-4"> <!-- Increased space between groups -->
        <div v-for="(group, groupIndex) in adminNavigationGroups" :key="groupIndex">
          <!-- Group Heading -->
          ...
          <div class="space-y-1">
            <NuxtLink
              ...
              :to="item.path"
              :class="[
                'flex items-center px-4 py-2.5 rounded-lg transition-colors text-sm', // Adjusted text size
                route.path.startsWith(item.path)
                  ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-medium' // Added font-medium for active
                  : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'
              ]"
            >
              <UIcon :name="item.icon" class="text-lg flex-shrink-0" /> <!-- Use UIcon -->
              <span class="ml-3">{{ item.name }}</span>
              <!-- Badge logic remains the same if needed -->
              <span
                v-if="item.badge"
                :class="[item.badgeColor || 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200', 'ml-auto text-xs rounded-full px-1.5 py-0.5']"
              >
                {{ item.badge }}
              </span>
            </NuxtLink>
...
const { adminNavigationGroups } = useAdminNavigation(); // Get grouped admin navigation
